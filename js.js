...
sortForm.addEventListener("change", function (event) {
    ...
    switch (sortForm['sort'].value) {
        ...
        case "defense":
            sortedPokemons = sortedPokemons.sort(function (first, second) {
                if (first.stats[2].base_stats > second.stats[2].base_stat) {
                    return order
                }
...
button.addEventListener("click", function() {
    ...
    if (attackFromSlider.value >= 5){
        sortedPokemons = sortedPokemons.filter(function(pokemon){
            return pokemon.stats[0].base_stat >= attackFromSlider.value
        })
    }

    if (attackToSlider.value <= 130){
        sortedPokemons = sortedPokemons.filter(function(pokemon){
            return pokemon.stats[0].base_stat <= attackToSlider.value
        })
    }


    if (defenseFromSlider.value >= 5){
        sortedPokemons = sortedPokemons.filter(function(pokemon){
            return pokemon.stats[0].base_stat >= defenseFromSlider.value
        })
    }

    if (defenseToSlider.value <= 180){
        sortedPokemons = sortedPokemons.filter(function(pokemon){
            return pokemon.stats[0].base_stat <= defenseToSlider.value
        })
    }
    redrawSortedPokemons()
})
...
let defenseFromText = document.getElementById("defense-from-text")
let defenseFromSlider = document.getElementById("defense-from")

defenseFromSlider.addEventListener("input", function () {
    defenseFromText.textContent = defenseFromSlider.value
    defenseToSlider.setAttribute("min", defenseFromSlider.value)
})

let defenseToText = document.getElementById("defense-to-text")
let defenseToSlider = document.getElementById("defense-to")

defenseToSlider.addEventListener("input", function () {
    defenseToText.textContent = defenseToSlider.value
    defenseFromSlider.setAttribute("max", defenseToSlider.value)
})
